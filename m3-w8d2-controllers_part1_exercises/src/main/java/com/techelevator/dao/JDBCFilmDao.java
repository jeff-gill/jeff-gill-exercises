...
public class JDBCFilmDao implements FilmDao {
	...
	@Override
	public List<Film> getFilmsBetween(String category, int minLength, int maxLength) {
		...
		SqlRowSet results = jdbcTemplate.queryForRowSet(filmSearchSql, category, minLength, maxLength);
		while (results.next()) {
	...
	public List<String> getCategoryName() {
		...
		String categorySearchSql = "SELECT name FROM category ORDER BY name";
		SqlRowSet resultCategory = jdbcTemplate.queryForRowSet(categorySearchSql);
		while (resultCategory.next()) {
			categoryName.add(mapRowToCategory(resultCategory));
		}
		return categoryName;
	}
	...
	private String mapRowToCategory(SqlRowSet resultCategory) {
		return resultCategory.getString("name");
	}
